...
namespace HP_Parking_WEB_API
{
    ...
    public class HPParking : IHPParking
    {
            ...
            try
            {
                ...
                if (dt.Tables[0].Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < dt.Tables[0].Rows.Count; i++)
                    {
                        ...
                        objPark.Rating = GetparkingRatings(dt.Tables[0].Rows[i]["ParkingLocationId"].ToString().Trim());
                        objPark.Remarks = "";
        ...
        private string getParkMeRequest(ParkinRequest ParkMeRequest)
        {
            ...
            if (DeletePriviousRequst(ParkMeRequest) == true)
            {
                ...
                try
                {
                    ...
                    try
                    {
                        cmd.ExecuteNonQuery(); message = "Your Park Me request has been sent to concerned parking. Contact the parking operator.";
                        dbConnection.Close(); cmd.Dispose();
                    }
                    catch (Exception ex)
                    {
        ...
        private DataTable CheckParkingStatus(string Mobilenumber, string Vehiclenumber)
        {
            bool Parked = false;
            dbConnection = DBConnect.getConnection();
            ...
            //  double time=Convert.ToDouble(Convert.ToDateTime(Time).ToString("HH:mm"));
            DataTable dt = new DataTable();
            SqlCommand cmd = new SqlCommand("SP_GetParkingStatus", dbConnection);
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Parameters.Clear();
        ...
        private string getParkOutRequest(ParkinRequest ParkOutRequest)
        {
            ...
            string paringid="";
            DataTable dt = new DataTable();
            dt = CheckParkingStatus(ParkOutRequest.PhoneNumber.ToString().Trim(),ParkOutRequest.VehicleNo.ToString().Trim());
            if (dt.Rows.Count > 0)
            {
                paringid = dt.Rows[0]["ParkingId"].ToString().Trim();
                ...
                try
                {
                    ...
                    cmd.Parameters.AddWithValue("@ParkingId", paringid);
                    ...
                    {
                        cmd.ExecuteNonQuery(); 
                        message = "Your Check Out request has been sent to concerned parking. Please contact the parking operator at "+ GetparkingName(paringid.Trim());
                        dbConnection.Close(); cmd.Dispose();
                    }
                    catch (Exception ex)
                    {
                        message = "Request failed, please try again" + ex.Message.ToString();
                        dbConnection.Close(); cmd.Dispose();
                    }
                }
                catch (Exception ex)
            ...
            {
                message = "Your request can not be processed as your vehicle is not entered in any Parking Location.";
            }
            return message;
        }
        ...
        #region Rating to the Parking
        private string GetparkingRatings(string ParkingId)
        {
            string Ratings = "1.0";
                ...
                DataTable dt = new DataTable();
                dbConnection = DBConnect.getConnection();
                SqlCommand cmd = new SqlCommand("SP_GetParkingRating", dbConnection);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.Clear();
                cmd.Parameters.AddWithValue("@ParkingId", ParkingId);
                ...
                try
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        int TotalRation=Convert.ToInt32(dt.Rows[0]["Rate"].ToString().Trim());
                        int Count=Convert.ToInt32(dt.Rows[0]["Count"].ToString().Trim());
                        Ratings = string.Format("{0:0.00}", ((Double)TotalRation/Count).ToString());
                        if (Convert.ToInt32(Ratings) > 5)
                        {
                            Ratings = "5.00";
                        }
                    }
        ...
        public string getParkingRateing_JSON(string ParkingId)
        {
            return GetparkingRatings(ParkingId);
        }

        public string getParkingRateing_XML(string ParkingId)
        {
            return GetparkingRatings(ParkingId);
        }
        #endregion
